...
begin
  ...
  if v_current_role = 'consorcio_admin' then
    return exists (
      ...
      where profile_id = countrify.uid()
...
begin
  ...
  actor_role := countrify.complaint_actor_role_for_profile(countrify.uid());
  actor_label := case
    ...
    else countrify.complaint_actor_label_for_profile(countrify.uid())
  end;
...
as $$
  ...
  left join countrify.iadmin_role_grants g
   ...
   and g.profile_id = countrify.uid()
  where countrify.current_user_role() = 'super_admin' or g.profile_id = countrify.uid()
$$;
...
as $$
  select coalesce(
    (select p.building_id from countrify.profiles p where p.id = countrify.uid() limit 1),
    (
      select m.building_id
      from countrify.unit_profile_memberships m
      where m.profile_id = countrify.uid()
        and m.active
      order by
...
as $$
  select exists (
    ...
    where m.unit_id = target_unit_id
      and m.profile_id = countrify.uid()
      and m.active
  )
$$;
