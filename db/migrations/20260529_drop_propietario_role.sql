...
with promote as (
  select m.id
   ...
   where m.relationship_type = 'propietario'
     and not exists (
       select 1
         from countrify.unit_profile_memberships m2
...
update countrify.unit_profile_memberships m
   set relationship_type = 'vecino_principal'::countrify.unit_profile_relationship,
       is_primary = false
  from promote
...
update countrify.profiles
   set role = 'vecino'::countrify.app_role
