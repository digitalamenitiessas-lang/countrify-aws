...
export type GenerateRemindersCoreInput = {
  administrationId: string
  propertyId?: string | null
  daysBeforeDue?: number
  /**
   * Profile id del actor (admin que dispara). Pasar null cuando lo corre el
   * cron sistémico — el audit log queda con actor_profile_id = NULL.
   */
  actorProfileId: string | null
}

export type GenerateRemindersCoreResult = {
  created: number
  skipped: number
}
...
/**
 * Core de generación de recordatorios. Sin auth ni user context.
 * El action user-scoped (`generateReminders`) y el cron sistémico
 * (`/api/cron/generate-reminders`) llaman acá adentro.
 */
export async function generateRemindersCore(
  input: GenerateRemindersCoreInput,
): Promise<GenerateRemindersCoreResult> {
  const daysBeforeDue = input.daysBeforeDue ?? 3
  ...
  const flatRows = await listReminderRunsWithItemsFromPostgres({
    ...
    managedPropertyId: input.propertyId ?? null,
  })
  ...
  await insertIAdminAuditLogInPostgres({
    ...
    entityId: null,
    action: input.actorProfileId ? 'reminders.generated' : 'reminders.generated_by_cron',
    metadata: {
      created,
      skipped,
      property_id: input.propertyId ?? null,
      source: input.actorProfileId ? 'manual' : 'cron',
    },
  })
